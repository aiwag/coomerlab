...
interface SettingsState {
  sidebarVisible: boolean;
  sidebarCollapsed: boolean;
  magicGrid: boolean; // Replaces gridSize and autoMode
  bandwidthMode: 'low' | 'medium' | 'high';
  browserVisible: boolean;
  
  toggleSidebar: () => void;
  toggleSidebarCollapse: () => void;
  toggleMagicGrid: () => void;
  setBandwidthMode: (mode: 'low' | 'medium' | 'high') => void;
  setBrowserVisible: (visible: boolean) => void;
}
...
export const useSettingsStore = create<SettingsState>()(
  persist(
    (set) => ({
      sidebarVisible: true,
      sidebarCollapsed: false,
      magicGrid: true, // Enable the pro grid by default
      bandwidthMode: 'medium',
      browserVisible: true,
      
      toggleSidebar: () => set((state) => ({ sidebarVisible: !state.sidebarVisible })),
      toggleSidebarCollapse: () => set((state) => ({ sidebarCollapsed: !state.sidebarCollapsed })),
      toggleMagicGrid: () => set((state) => ({ magicGrid: !state.magicGrid })),
      setBandwidthMode: (mode) => set({ bandwidthMode: mode }),
      setBrowserVisible: (visible) => set({ browserVisible: visible }),
    }),
    {
      name: 'camviewer-settings-storage-v3', // New version to reset old settings
    }
  )
);